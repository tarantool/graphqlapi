...
local _graphql_cache_size = {}

local function cache_reset(schema, new_size)
    schema = utils.coerce_schema(schema)
    new_size = new_size or _graphql_cache_size
        or defaults.GRAPHQL_QUERY_CACHE_SIZE

    _graphql_cache[schema] = {}
    _graphql_cache_size[schema] = new_size
end

-- TODO: implement lru_list with limit
local function cache_set(schema, request_hash, request_ast)
    ...
    schema = utils.coerce_schema(schema)
    if _graphql_cache[schema] == nil then
        _graphql_cache[schema] = {}
        cache_reset(schema)
    end
    _graphql_cache[schema][request_hash] = request_ast
end
...
local function cache_get(schema, request_hash)
    ...
    return _graphql_cache[schema][request_hash]
end
